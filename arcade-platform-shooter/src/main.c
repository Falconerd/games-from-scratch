...
static void reset() {
	...
	context.box_spawn_timer = -1;
	context.is_box_spawned = 0;
...
int main(void) {
	...
	while (!glfwWindowShouldClose(render_context.window)) {
		...
		// Handle user input and player movement.
		{
			...
			if (glfwGetKey(render_context.window, GLFW_KEY_J) == GLFW_PRESS) {
			}
			...
			if (glfwGetKey(render_context.window, GLFW_KEY_E) == GLFW_PRESS) {
				if (context.shoot_timer <= 0) {
					...
					case WT_MACHINE_GUN: {
						...
						spawn_projectile(PT_BULLET, player->aabb.position[0], player->aabb.position[1], 400, 0, 9, on_bullet_collide, on_bullet_collide_static);
						context.weapon_kick = 100;
						render_screen_shake_add(0.05f, 0.15f);
					} break;
					case WT_SHOTGUN: {
						context.shoot_timer = 0.75f;
						context.weapon_kick = 200;
